...
export default function DashboardClient({ targetUserId, isAdminView, profile, initialTiers, initialScript, initialLeads }) {
  ...
  const doAddLead = () => {
    ...
    startTransition(async () => {
      const res = await addLead(targetUserId, phoneInput);
      if (res?.error) {
        setLeadError(res.error);
      } else {
        setLeadError("");
        setPhoneInput("");
        setLeads((l) => [{ id: crypto.randomUUID(), phone: phoneInput, status: "nuevo", timezone: "china", paused: false, updated_at: new Date().toISOString() }, ...l]);
      }
    });
  };
